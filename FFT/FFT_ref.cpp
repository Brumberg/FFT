...
void CFFT<loglen, TW, T, TFFT>::CalculateRealIFFT(TFFT buffer[], TFFT ffttransform[], EN_ScalingMethod rescale, size_t *pBlockExponent)
{
	...
	*pBlockExponent = 0;
	...
	/*if (rescale==SCALE_INPUT||rescale==SCALEINPOUT)
	{
		TFFT::DATATYPE MaxVal1	= abs(ffttransform[0].re);
		TFFT::DATATYPE MaxVal2	= abs(ffttransform[0].im);
		TFFT::DATATYPE MaxVal	= MaxVal1>=MaxVal2?MaxVal1:MaxVal2;
		for (size_t i=1;i<length;i++)
		{
			TFFT::DATATYPE MaxVal1 = abs(ffttransform[i].re);
			TFFT::DATATYPE MaxVal2 = abs(ffttransform[i].im);
			TFFT::DATATYPE dummy   = MaxVal1>=MaxVal2?MaxVal1:MaxVal2;
			if (dummy>MaxVal)
			{
				MaxVal=dummy;
			}
...
void CFFT<loglen, TW, T, TFFT>::Convert2ComplexDFT(TFFT transform[], size_t exponent)
{
	...
	//float lval					= transform[0].re+transform[0].im;
	//float hval					= transform[0].re-transform[0].im;
	typename TFFT::OFWTYPE lval		= (static_cast<typename TFFT::OFWTYPE>(transform[0].re)+transform[0].im)<<(exponent-1);
	typename TFFT::OFWTYPE hval		= (static_cast<typename TFFT::OFWTYPE>(transform[0].re)-transform[0].im)<<(exponent-1);
	//transform[0].re				= 0.5f*lval;
	//transform[0].im				= 0.5f*hval;
