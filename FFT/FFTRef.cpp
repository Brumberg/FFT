...
void CFFT<loglen, TW, T, TFFT>::CalculateIFFT(TFFT buffer[], TFFT ffttransform[], EN_ScalingMethod rescale, size_t *pBlockExponent)
{
	...
	*pBlockExponent = 0;
	...
	if (rescale==SCALE_INPUT||rescale==SCALEINPOUT)
	{
		TFFT::DATATYPE MaxVal1		= abs(buffer[0].re);
		TFFT::DATATYPE MaxVal2		= abs(buffer[0].im);
		TFFT::DATATYPE MaxVal		= MaxVal1>MaxVal2?MaxVal1:MaxVal2;
		for (size_t i=1;i<length;i++)
		{
			TFFT::DATATYPE dummy1 = abs(buffer[i].re);
			TFFT::DATATYPE dummy2 = abs(buffer[i].im);
			TFFT::DATATYPE dummy  = dummy1>dummy2?dummy1:dummy2;
			if (dummy>MaxVal)
			{
				MaxVal=dummy;
			}
...
void CFFT<loglen, TW, T, TFFT>::Convert2HalfDFT(TFFT transform[], size_t exponent)
{
	...
	typename TFFT::OFWTYPE lval			= (static_cast<typename TFFT::OFWTYPE>(transform[0].re)+transform[0].im)<<exponent;
	typename TFFT::OFWTYPE hval			= (static_cast<typename TFFT::OFWTYPE>(transform[0].re)-transform[0].im)<<exponent;
	transform[0].re						= static_cast<typename TFFT::DATATYPE>(lval>>1);
	transform[0].im						= static_cast<typename TFFT::DATATYPE>(hval>>1);
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	{
		//Signal[i] = 2.e4*sin(6.283185307179586476925286*i/1024*2)+5000*cos(6.283185307179586476925286*i/1024*4);
		fSignal[i]=10;//(10*cos(6.283185307179586476925286*i/1024*512));//floor(2.e4*sin(6.283185307179586476925286*i/1024*2)+5000*cos(6.283185307179586476925286*i/1024*4));//sin(6.283185307179586476925286*i/1024*2)+5000*cos(6.283185307179586476925286*i/1024*4);
		dfSignal[i]=10*(cos(6.283185307179586476925286*i/1024*257));//2*sin(3*(6.283185307179586476925286*i)/1024)+4*cos(5*(6.283185307179586476925286*i)/1024)+0.3;//1*sin(257*(6.283185307179586476925286*i)/1024);/*+5000*cos(6.283185307179586476925286*i/1024*4))*///sin(6.283185307179586476925286*i/1024*2)+5000*cos(6.283185307179586476925286*i/1024*4);
		Signal[i]=2e9*sin(3*(6.283185307179586476925286*i)/1024)+4e6*cos(5*(6.283185307179586476925286*i)/1024)+50e3;
	}
	//Signal[0]=1E9;
	//dfSignal[0]=1e9;
	...
	m_FFT.CalculateFFT(Signal,TransformScaling,SCALEINPOUT,&Exponent1);
	m_FFT.CalculateIFFT(TransformScaling,Transform,SCALEINPOUT,&Exponent2);
...
Electrical Engineer Ultrasonic Flow Measurement R&D 
Endress+Hauser Flowtec AG | Kaegenstrasse 7 | CH-4153 Reinach BL | Switzerland
Phone: +41 61 715 6526
oliver.brumberg@flowtec.endress.com | www.flowtec.endress.com
